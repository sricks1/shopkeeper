  ...
  if (compact) {
    return status === "in_stock" ? (
      <button
        ...
        onClick={reorder}
        disabled={busy}
        className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-[#e06829] px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-[#c55a22] disabled:opacity-60"
      >
      ...
      <button
        ...
        onClick={markInStock}
        disabled={busy}
        className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-emerald-700 disabled:opacity-60"
      >
        <Check size={13} />
        {busy ? "…" : "In Stock"}
      </button>
    ...
    <div className="rounded-xl bg-white px-4 py-4 shadow-sm ring-1 ring-zinc-200">
      <div className="flex items-center justify-between gap-3">
        ...
        {status === "in_stock" ? (
          <button
            ...
            onClick={reorder}
            disabled={busy}
            className="inline-flex items-center gap-2 rounded-lg bg-[#e06829] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#c55a22] disabled:opacity-60"
          >
      ...
      {status === "on_order" && (
        <p className="mt-2 text-xs text-zinc-400">
          Ordered — there's an order task on the board. Tap “In Stock” when it arrives.
        </p>
      )}
      {error && (
