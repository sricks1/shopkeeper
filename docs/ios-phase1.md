...
- **There is no dev database.** Supabase has one ShopKeeper project and it is production; `.env.local` and `.env.production` both point at it, by choice (2026-08-19), since only Steven and Flash use the system. The Debug/Release xcconfig split is therefore about build configuration, not data isolation. Practical consequence: anything you do in the Simulator writes to the live shop. Scanning a real QR label works correctly, because there is only one database behind it.
- **Inventory writes are unsafe from this app.** `sync_order_stock_status` is a trigger on `staff_tasks` that writes `inventory_items.stock_status` one-directionally. Nothing syncs the other way, so writing stock status from iOS would desync the web orders board. Either iOS grows the order/task model (out of scope, and that schema is still moving) or a write path needs designing. Open question for Steven.
- **`repair_consumables` no longer affects stock.** Its decrement trigger was dropped with the quantity columns; PRD 4.2 step 4 is stale. Recording consumables on a repair is service history only.
- **PostgREST `order()` on an embedded resource** sorts rows *inside* the embed, not the parent rows. Sort joined lists client-side.
- **Notification payloads are heterogeneous per type** and some are legacy-shaped (`reorder_needed` still carries `quantity_on_hand`/`reorder_threshold`, columns that no longer exist). Decode defensively, never into one fixed struct.
